...
alias (`cargo doido` is equivalent to `cargo run --bin economizei`):
...
| `cargo doido db seed` | Insert development fixture data |
| `cargo doido db prepare` | Load `db/schema.sql` when the database is empty |
...
│   ├── schema.sql   ← canonical schema snapshot (reset/prepare)
│   └── seed/        ← Rust seed runner (`doido db seed`)
└── tests/
```
