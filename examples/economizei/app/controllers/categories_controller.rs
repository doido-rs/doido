use crate::models::category::{ActiveModel, Column, Entity, Model};
...
impl CategoriesController {
    pub async fn index(mut ctx: Context) -> doido::Result<Response> {
        ...
        let base_path = listing::collection_path("categories");
        Ok(listing::respond_index(
            &ctx,
            "categories/index",
            &i18n::t("resources.categories"),
            &base_path,
    ...
    pub async fn show(mut ctx: Context) -> doido::Result<Response> {
        ...
        match Entity::find_by_id(parse_id(&ctx)).one(ctx.db()).await? {
            Some(row) if row.company_id == company_id => {
                ...
                let fields = associations::category_show_fields(&row, &label_map);
                let base_path = listing::collection_path("categories");
    ...
    pub async fn create(mut ctx: Context) -> doido::Result<Response> {
        ...
        let form: CategoryForm = listing::parse_json_or_form(ctx).await?;
        ...
        let redirect = listing::collection_path("categories");
...
fn categories_to_csv(rows: Vec<Model>) -> String {
    csv::build_csv(
        &["id", "company_id", "name"],
        rows.into_iter()
            .map(|row| {
