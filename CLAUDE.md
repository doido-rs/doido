...
- [x] 05-config — **Per-env YAML (`config/<env>.yml`), env via Tera `{{ get_env(name=…, default=…) }}` on every load (`doido_core::config::render`), optional `doido config render`, AES-256-GCM credentials; app `settings:` + `Doido::before_run`; layered TOML dropped (US-085)**
