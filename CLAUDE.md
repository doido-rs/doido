...
- [x] 06b-generators — **Separate crate, all Rails targets, `Generator` trait registry, `generate helper`, scaffold/controller emit helpers, auto-injects `config/routes.rs`. App-registrable generators: any `doido::Generator` (defined in the app or a third-party crate) installs via the `doido::Doido` builder in `src/main.rs` (`.register_generator(...)`, threaded through `commands::generate::run_with`) and is dispatched by `cargo doido generate` alongside built-ins. `doido extension <Name>` scaffolds publishable extension crates (`doido-<snake>/`) with generators harness.**
