...
- [x] 05-config — **Per-env YAML (`config/<env>.yml`), AES-256-GCM encrypted credentials (`credentials.yml.enc` + `master.key`), in-YAML env vars via Tera `{{ get_env(name=…, default=…) }}` (`doido_core::config::render`, applied by every crate's `from_yaml`; `SECTION__KEY` overrides removed); layered TOML dropped (US-085)**
- [x] 06-cli — **Runtime commands only; `doido generate` delegates to `doido-generators`**
- [x] 06b-generators — **Separate crate, all Rails targets, `Generator` trait registry, `generate helper`, scaffold/controller emit helpers, auto-injects `config/routes.rs`. App-registrable generators: any `doido::Generator` (defined in the app or a third-party crate) installs via the `doido::Doido` builder in `src/main.rs` (`.register_generator(...)`, threaded through `commands::generate::run_with`) and is dispatched by `cargo doido generate` alongside built-ins.**
...
- [x] 16-auth — **`AuthUser` trait, axum extractors (`CurrentUser`/`MaybeUser`/`RequireAuth`/`AuthToken`), pluggable strategies (cookie/JWT/OAuth2), optional 2FA (`auth-2fa`), pre-built `routes::mount`. Devise-style `auth.modules` config (module-aware `auth:install`: per-module migration columns + `only:` routes; validatable/trackable behaviors live). Built-in-by-default: `doido new --auth`/`auth:install` copy no controllers/views — auth runs on framework built-in controllers + overridable framework views (`doido-view` template registry); `auth:controllers` ejects them (devise:controllers/views analogue). Generators in `doido-auth` (CLI-visible when installed). Release e2e: `auth_generators` + `auth_install` (`#[ignore]`, `make release-e2e`).**
