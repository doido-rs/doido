...
```rust
doido::view::init("app/views")?; // carga todos los *.html.tera bajo app/views
```

## Renderizar desde los controladores

Las actions renderizan vía [`ctx.render(...)`](@/docs/reference/controllers.es.md), que delega
...
solo en un miss — respaldado por cualquier [cache store](@/docs/reference/cache.es.md).
...
- [Controladores y enrutamiento](@/docs/reference/controllers.es.md) — `ctx.render(...)` y negociación de contenido.
- [Cache](@/docs/reference/cache.es.md) — el store detrás de la caché de fragmentos.
- [Mailer](@/docs/reference/mailer.es.md) — reutiliza este engine para las plantillas de correo.
