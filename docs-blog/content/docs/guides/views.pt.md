...
As actions renderizam via [`ctx.render(...)`](@/docs/guides/controllers.pt.md), que delega
...
em um miss — apoiado por qualquer [cache store](@/docs/guides/cache.pt.md).
...
- [Controllers & roteamento](@/docs/guides/controllers.pt.md) — `ctx.render(...)` e negociação de conteúdo.
- [Cache](@/docs/guides/cache.pt.md) — o store por trás do cache de fragmento.
- [Mailer](@/docs/guides/mailer.pt.md) — reutiliza esta engine para templates de e-mail.
