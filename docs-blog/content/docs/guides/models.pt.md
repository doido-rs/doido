...
[`ctx.db()`](@/docs/guides/controllers.pt.md).

```rust
// No boot:
let conn = doido_model::connect().await?;   // lê config/<env>.yml → [database]
...
> têm cada um seu módulo (`doido_model::{callbacks, scope, seeds, transaction, enums,
> tasks}`) espelhando seus equivalentes no Active Record.

## Veja também

- [Configuração](@/docs/guides/configuration.pt.md) — a seção `database`.
- [Controllers & roteamento](@/docs/guides/controllers.pt.md) — `ctx.db()` dentro das actions.
- [Geradores & CLI](@/docs/guides/generators.pt.md) — `doido generate model` e `doido db migrate`.
