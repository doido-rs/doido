...
```rust
doido_view::init("app/views")?; // load all *.html.tera under app/views
```

## Rendering from controllers

Actions render through [`ctx.render(...)`](@/docs/guides/controllers.md), which delegates to
...
`doido_view::helpers` provides Rails-style HTML helpers that return escaped HTML strings:
...
backed by any [cache store](@/docs/guides/cache.md).
...
- [Controllers & routing](@/docs/guides/controllers.md) — `ctx.render(...)` and content negotiation.
- [Cache](@/docs/guides/cache.md) — the store behind fragment caching.
- [Mailer](@/docs/guides/mailer.md) — reuses this engine for email templates.
