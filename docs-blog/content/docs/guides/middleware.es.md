...
servidor en cualquier [cache store](@/docs/guides/cache.es.md). `Session` guarda valores
tipados vía `set`/`get`.
...
[Controladores y enrutamiento](@/docs/guides/controllers.es.md).
...
[cache store](@/docs/guides/cache.es.md) como contador de respaldo.
...
- [Controladores y enrutamiento](@/docs/guides/controllers.es.md) — donde la stack envuelve tus rutas.
- [Cache](@/docs/guides/cache.es.md) — el store detrás de `CacheSessionStore` y el rate limiting.
- [Configuración](@/docs/guides/configuration.es.md) — la sección `middleware.cors`.
