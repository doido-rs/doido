...
through the same [Tera engine](@/docs/guides/views.md) as views.
...
onto the `mailers` queue for a [worker](@/docs/guides/jobs.md) to send.
...
- [Jobs](@/docs/guides/jobs.md) — the `mailers` queue and worker behind `deliver_later`.
- [Views](@/docs/guides/views.md) — the engine that renders email templates.
- [Generators & CLI](@/docs/guides/generators.md) — `doido generate mailer`.
