...
let app = MiddlewareStack::new()
    ...
    .with_allowed_hosts(vec!["doido.rs".into()]) // host authorization
...
state server-side in any [cache store](@/docs/guides/cache.md). `Session` holds typed
values via `set`/`get`.
...
See the "Strong parameters" section of [Controllers & routing](@/docs/guides/controllers.md).
...
[cache store](@/docs/guides/cache.md) as the backing counter.
...
- [Controllers & routing](@/docs/guides/controllers.md) — where the stack wraps your routes.
- [Cache](@/docs/guides/cache.md) — the store behind `CacheSessionStore` and rate limiting.
- [Configuration](@/docs/guides/configuration.md) — the `middleware.cors` section.
