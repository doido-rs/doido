...
reach the same pool through [`ctx.db()`](@/docs/guides/controllers.md).
...
- [Configuration](@/docs/guides/configuration.md) — the `database` section.
- [Controllers & routing](@/docs/guides/controllers.md) — `ctx.db()` inside actions.
- [Generators & CLI](@/docs/guides/generators.md) — `doido generate model` and `doido db migrate`.
