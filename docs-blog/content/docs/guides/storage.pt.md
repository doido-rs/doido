...
[doido-jobs](@/docs/guides/jobs.pt.md) em vez de bloquear a requisição.
...
- [Models](@/docs/guides/models.pt.md) — os attachments referenciam seus registros; `ensure_tables`.
- [Jobs](@/docs/guides/jobs.pt.md) — `purge_later` e limpeza em background.
- [Geradores & CLI](@/docs/guides/generators.pt.md) — `doido generate storage:install` / `storage:adapter`.
