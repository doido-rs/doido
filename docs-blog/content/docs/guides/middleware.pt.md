...
let app = MiddlewareStack::new()
    ...
    .with_allowed_hosts(vec!["doido.rs".into()]) // autorização de host
...
qualquer [cache store](@/docs/guides/cache.pt.md). `Session` guarda valores tipados via
`set`/`get`.
...
[Controllers & roteamento](@/docs/guides/controllers.pt.md).
...
[cache store](@/docs/guides/cache.pt.md) como contador de apoio.
...
- [Controllers & roteamento](@/docs/guides/controllers.pt.md) — onde a stack envolve suas rotas.
- [Cache](@/docs/guides/cache.pt.md) — o store por trás de `CacheSessionStore` e do rate limiting.
- [Configuração](@/docs/guides/configuration.pt.md) — a seção `middleware.cors`.
