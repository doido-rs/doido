...
[`ctx.db()`](@/docs/guides/controllers.es.md).

```rust
// En el arranque:
let conn = doido_model::connect().await?;   // lee config/<env>.yml → [database]
...
> tienen cada uno su módulo (`doido_model::{callbacks, scope, seeds, transaction, enums,
> tasks}`) reflejando sus equivalentes de Active Record.

## Véase también

- [Configuración](@/docs/guides/configuration.es.md) — la sección `database`.
- [Controladores y enrutamiento](@/docs/guides/controllers.es.md) — `ctx.db()` dentro de las actions.
- [Generadores y CLI](@/docs/guides/generators.es.md) — `doido generate model` y `doido db migrate`.
