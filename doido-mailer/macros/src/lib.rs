...
pub fn mailer(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    quote! {
        ...
        impl #ident {
            /// The snake_case mailer name (generated by `#[mailer]`).
            pub fn mailer_name() -> &'static str {
                #name
            }

            /// Template key for `action`: `mailers/<mailer_name>/<action>`.
            pub fn template_key(action: &str) -> String {
                format!("mailers/{}/{}", #name, action)
            }
        }
    }
    .into()
