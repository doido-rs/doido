...
git history, and `harness/progress.txt`. Do exactly one story, then stop.
...
5. Run `make verify`. It **must** exit 0. If it is red, fix it before doing anything
   else — never end an iteration with a red tree.
6. Update `harness/prd.json`: set this story's `"passes": true` and write a one-line
   `notes`. Append one line to `harness/progress.txt` (`US-XXX: <what changed>`).
7. Commit on branch `ralph/finish-doido`:
   `feat(<crate>): <story title> [US-XXX]`, ending with:
   `Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>`

## Hard rules

- **One story per iteration.** Do not start a second, even if there is time.
- **`make verify` green is the definition of done** for every story. `make verify`
  runs fmt + clippy + tests + the example app; it is deterministic and does not
