...
| `doido-model` | 03 | Done | sea-orm re-export + connection pool + Rails-style schema builders + db tasks (`seeds`, `tasks` reset/setup/prepare, `schema` dump/load, `migrate` rollback/redo, pool knobs) + `TestDb`. **Gap:** `testing::run_migrations()` / `testing::seed()` convenience helpers from the spec are not present (only `TestDb`). |
...
| `doido-generators` | 06, 06b | Done | CLI (`new`/`generate`/`server`/`console` via evcxr/`db`/`worker`) + generator registry + generators (model, controller, migration, scaffold, resource, mailer, job, channel, storage_install/adapter, templates, generator, locale) + route auto-injection + embedded templates. **Gaps:** `doido db` only exposes `create` + a SeaORM passthrough — `seed`/`reset`/`prepare`/`schema` exist in `doido-model` but are **not wired** as subcommands; `jobs:failed/retry/discard` are log-only stubs; `credentials:edit` is a log-only stub and `credentials:show` is absent; `server` does not parse `--port`/`--env`. |
...
4. **Small wiring / ergonomics.**
   - Wire `doido db seed/reset/prepare/schema` into the CLI (the `doido-model` tasks exist).
   - Real `doido jobs:failed/retry/discard` backed by the dead-letter queue.
   ...
   - `routes!` `HEAD`/`OPTIONS` verbs + route constraints (spec 01).
   - `testing::run_migrations()`/`seed()` helpers (spec 03).
